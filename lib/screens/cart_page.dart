...
class _CartPageState extends State<CartPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (!cart.isEmpty)
            TextButton(onPressed: _confirmClearCart, child: const Text('清空')),
        ],
      ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(cart.errorMessage ?? '購物車載入失敗', textAlign: TextAlign.center),
            const SizedBox(height: 16),
            FilledButton(
              onPressed: () {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            FilledButton(
              onPressed: _checkout,
              child: Text('結算 (${cart.totalQuantity})'),
            ),
          ],
        ),
      ),
  ...
  Future<void> _confirmClearCart() async {
    ...
    await _runCartAction(() => context.read<CartProvider>().clearCart());
  }
  ...
  Future<void> _runCartAction(Future<void> Function() action) async {
    try {
      ...
      final message = context.read<CartProvider>().errorMessage ?? '購物車操作失敗';
