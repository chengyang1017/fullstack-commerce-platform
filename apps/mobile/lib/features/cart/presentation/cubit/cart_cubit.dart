...
class CartCubit extends Cubit<CartState> {
  ...
  Future<void> _loadCart() async {
    emit(CartState(status: CartStatus.loading, items: state.items));

    try {
      final items = await _repository.loadCart();

      emit(
        CartState(status: CartStatus.ready, items: List.unmodifiable(items)),
      );
      ...
      emit(
        const CartState(
          ...
          errorType: CartErrorType.loadFailed,
        ),
      );
    }
  }
  ...
  Future<void> addProduct(Product product) {
    return _enqueue(() async {
      ...
      final nextItems = List<CartItem>.from(state.items);
  ...
  Future<void> removePurchasedItems(Iterable<CartItem> purchasedItems) {
    return _enqueue(() async {
      await loadCart();
      ...
      for (final currentItem in state.items) {
        ...
        final remainingQuantity = currentItem.quantity - purchasedQuantity;

        if (remainingQuantity > 0) {
          nextItems.add(currentItem.copyWith(quantity: remainingQuantity));
        }
      }
  ...
  Future<void> clearCart() {
    return _enqueue(() async {
      ...
      emit(const CartState(status: CartStatus.ready, items: []));
      ...
      try {
        ...
        emit(
          CartState(
            status: CartStatus.error,
            items: previousItems,
            errorType: CartErrorType.clearFailed,
          ),
        );
  ...
  Future<void> _saveOptimistically(List<CartItem> nextItems) async {
    ...
    // 先更新 UI。
    emit(
      CartState(status: CartStatus.ready, items: List.unmodifiable(nextItems)),
    );

    try {
      await _repository.saveCart(state.items);
    } catch (error, stackTrace) {
      developer.log(
      ...
      emit(
        CartState(
          status: CartStatus.error,
          items: previousItems,
          errorType: CartErrorType.updateFailed,
        ),
      );
  ...
  int _findProductIndex(String productId) {
    return state.items.indexWhere((item) => item.product.id == productId);
  }
}
